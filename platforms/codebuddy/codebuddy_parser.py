"""
CodeBuddy平台解析器 - 解析CodeBuddy格式的Skill文件
================================================

CodeBuddy的skill文件结构：
- 位置: ~/.codebuddy/skills/{skill-name}/SKILL.md
- 格式: Markdown + YAML前言
- 变量: $ARGUMENTS 或其他
...
CodeBuddy skill特点：
1. 与WorkBuddy格式完全兼容
2. 支持中英文双语描述（description_zh, description_en）
3. 支持更丰富的元数据字段
4. 变量语法与Claude类似
5. 工具权限在YAML前言中定义
...
class CodeBuddyParser(BaseParser):
    ...
    解析CodeBuddy格式的skill文件，转换为通用Skill格式。
    规则与WorkBuddy完全一致。
    """
    ...
    def parse_file(self, file_path: Union[str, Path]) -> UniversalSkill:
        """
        解析单个CodeBuddy skill文件
        
        CodeBuddy skill文件通常是SKILL.md，包含YAML前言和Markdown内容。
        ...
        path = Path(file_path)
        if not path.exists():
            raise FileNotFoundError(f"CodeBuddy skill file not found: {file_path}")
        ...
        skill = UniversalSkill(
            ...
            variables=variables,
            source_platform=SkillPlatform.CODEBUDDY,
        ...
        if platform_overrides:
            skill.set_platform_override(SkillPlatform.CODEBUDDY, platform_overrides)
        
        # 验证skill
        if not self.validate_skill(skill):
            errors = self.get_errors()
            raise ValueError(f"Invalid CodeBuddy skill: {'; '.join(errors)}")
        
        return skill
    
    def parse_directory(self, dir_path: Union[str, Path]) -> UniversalSkill:
        """
        解析整个CodeBuddy skill目录
        ...
        path = Path(dir_path)
        if not path.exists():
            raise FileNotFoundError(f"CodeBuddy skill directory not found: {dir_path}")
    ...
    def extract_tools(self, yaml_data: Dict[str, Any]) -> List[ToolPermission]:
        ...
        CodeBuddy在YAML前言的allowed-tools字段中定义工具权限。
        ...
        for tool_name in allowed_tools:
            ...
            tool = ToolPermission(
                name=tool_name,
                category=category,
                description=f"CodeBuddy tool: {tool_name}"
            )
            tools.append(tool)
...
register_parser(CodeBuddyParser())
