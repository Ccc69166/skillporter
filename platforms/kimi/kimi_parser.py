"""
Kimi Code Agent平台解析器 - 解析Kimi Code Agent格式的Skill文件
================================================

Kimi Code Agent的skill文件结构：
- 位置: ~/.kimi/skills/{skill-name}/SKILL.md
- 格式: Markdown + YAML前言
- 变量: $ARGUMENTS, $CONTEXT等
- 工具: 在YAML前言的allowed-tools字段中定义

Kimi Code Agent skill特点：
...
class KimiCodeAgentParser(BaseParser):
    """
    Kimi Code Agent Skill解析器
    
    解析Kimi Code Agent格式的skill文件，转换为通用Skill格式。
    规则与Claude Skills兼容。
    """
    ...
    def platform(self) -> SkillPlatform:
        """返回支持的平台类型"""
        return SkillPlatform.KIMI
    
    def parse_file(self, file_path: Union[str, Path]) -> UniversalSkill:
        """
        解析单个Kimi Code Agent skill文件
        
        Kimi Code Agent skill文件通常是SKILL.md，包含YAML前言和Markdown内容。
        ...
        path = Path(file_path)
        if not path.exists():
            raise FileNotFoundError(f"Kimi Code Agent skill file not found: {file_path}")
        ...
        skill = UniversalSkill(
            ...
            variables=variables,
            source_platform=SkillPlatform.KIMI,
        ...
        if platform_overrides:
            skill.set_platform_override(SkillPlatform.KIMI, platform_overrides)
        
        # 验证skill
        if not self.validate_skill(skill):
            errors = self.get_errors()
            raise ValueError(f"Invalid Kimi Code Agent skill: {'; '.join(errors)}")
        
        return skill
    
    def parse_directory(self, dir_path: Union[str, Path]) -> UniversalSkill:
        """
        解析整个Kimi Code Agent skill目录
        
        Kimi Code Agent skill目录结构：
        {skill-name}/
        ├── SKILL.md           # 主skill文件
        ├── scripts/           # 脚本目录（可选）
        │   └── *.py, *.sh等
        ...
        path = Path(dir_path)
        if not path.exists():
            raise FileNotFoundError(f"Kimi Code Agent skill directory not found: {dir_path}")
    ...
    def extract_variables(self, content: str) -> List[Variable]:
        ...
        Kimi Code Agent支持$VARIABLE_NAME格式的变量。
    ...
    def extract_tools(self, yaml_data: Dict[str, Any]) -> List[ToolPermission]:
        ...
        Kimi Code Agent在YAML前言的allowed-tools字段中定义工具权限。
        ...
        for tool_name in allowed_tools:
            ...
            tool = ToolPermission(
                name=tool_name,
                category=category,
                description=f"Kimi Code Agent tool: {tool_name}"
            )
            tools.append(tool)
...
register_parser(KimiCodeAgentParser())
