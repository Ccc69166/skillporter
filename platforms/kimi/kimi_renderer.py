"""
Kimi Code Agent平台渲染器 - 渲染Skill为Kimi Code Agent格式
================================================

将通用Skill格式转换为Kimi Code Agent可识别的格式。

Kimi Code Agent skill格式要求：
- 主文件: SKILL.md
...
class KimiCodeAgentRenderer(BaseRenderer):
    """
    Kimi Code Agent Skill渲染器
    
    将通用Skill格式渲染为Kimi Code Agent可识别的格式。
    规则与Claude Skills兼容。
    """
    ...
    def render_to_file(self, skill: UniversalSkill, file_path: Union[str, Path]) -> None:
        ...
        对于Kimi Code Agent，这会生成一个单独的SKILL.md文件。
    ...
    def _prepare_yaml_frontmatter(self, skill: UniversalSkill) -> Dict[str, Any]:
        ...
        # 平台特有字段
        kimi_overrides = skill.get_platform_override(SkillPlatform.KIMI)
        if kimi_overrides:
            yaml_data.update(kimi_overrides)
    ...
    def get_variable_mapping(self) -> Dict[str, str]:
        ...
        Kimi Code Agent使用$VARIABLE_NAME格式。
    ...
    def get_tool_mapping(self) -> Dict[str, str]:
        ...
        Kimi Code Agent使用与Claude类似的工具名称。
    ...
    def get_path_mapping(self) -> Dict[str, str]:
        ...
        Kimi Code Agent使用~/.kimi/skills/路径。
        ...
        return {
            ...
            "~/.codex/skills/": "~/.kimi/skills/",
            "~/.workbuddy/skills/": "~/.kimi/skills/"
        }
    
    def platform_specific_validation(self, skill: UniversalSkill) -> List[str]:
        """
        Kimi Code Agent平台特定的验证逻辑
        ...
        for var in skill.variables:
            if not var.placeholder.startswith("$"):
                errors.append(f"Variable '{var.name}' must use $ prefix for Kimi Code Agent")
...
register_renderer(KimiCodeAgentRenderer())
